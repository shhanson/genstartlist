...
const optionDefinitions = [
    ...
    {name: 'sort_by_categories', alias: 'c', type: Boolean},
    {name: 'output_file', alias: 'o', type: String, defaultValue: "./files/startList.csv"},
];
...
let INPUTFILEPATH = path.join(options.input_file);
...
let rawSheet = spreadsheetFile.SheetNames[0];
let rawSheetData = spreadsheetFile.Sheets[rawSheet];
let data = xlsx.utils.sheet_to_json(rawSheetData);
...
let sessionHeader = [ ['Lot #', 'USAW #', 'Year of Birth', 'Division', 'Weight Class', 'First Name', 'Last Name', 'Snatch', 'C&J', 'Club', 'Coach'] ];
let emptyRow = ['', '', '', '', '', '', '', '', '', '', ''];
...
for(let i = 0; i < allSorted.length; i++){
    let athlete = allSorted[i];
    let row = [' ', athlete.usawID, athlete.birthYear, DIVISIONS[athlete.division], CATEGORIES[athlete.category], athlete.firstName, athlete.lastName, athlete.snatchOpener, athlete.cjOpener, athlete.club, athlete.coach];
